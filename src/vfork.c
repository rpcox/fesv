/*
 *  vfork.c - create a child process and block the parent
 *
 */
...
#include <unistd.h>     /* vfork(2), getpid(2), sleep(3) */
...
int main() {
  ...
  /*  The call to vfork(2) below will return 1 of 3 values.
   *    -1 => the fork failed
   *     0 => the fork return value for the child,
   *     N => the fork return value for the parent (PID of the child)
   */

  switch ( pid = vfork() ) {    /* The only diff with fork.c               */
